...
원본 프로젝트 `DataDynamics/hive-query-audit-hook`에서 다음 사항을 보정했습니다.

| 항목 | 원본 | 보정 |
|------|------|------|
| 패키지명 | `io.datadynamics.hive.hook` | `org.opencloudengine.argus.catalog.collector.hql.hook` |
| groupId | `io.datadynamics.hive` | `org.opencloudengine.argus.catalog.collector` |
| artifactId | `hive-query-audit-hook` | `hive-query-hook` |
| JDK 요구사항 | README에 JDK 8로 기재 (실제 코드는 JDK 11 API 사용) | JDK 11+로 정정 |
| targetUrl null 체크 | 없음 (NullPointerException 가능) | URL 미설정 시 경고 로그 후 return |
| PRE_EXEC_HOOK 전송 | 로그만 출력, 서버 전송 안함 | REST API로 `RUNNING` 상태 전송 |
| operationName | 수집했으나 전송 payload에 미포함 | 모든 Hook 타입의 payload에 포함 |
| durationMs 타입 | `long` (Map 내 타입 혼재) | `String`으로 통일 (JSON 직렬화 일관성) |
